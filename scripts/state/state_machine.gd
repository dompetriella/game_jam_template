...
func _ready() -> void:
	...
	if (default_state != null):
		print_debug('Entering State: ' + states.find_key(default_state))	
		default_state.enter();
...
func _on_transition_state(old_state: State, new_state_name: String):
	...
	if (!new_state):
		print_debug("WARNING: No state found with name " + new_state_name + ". Exiting...")
		return;
	
	if (new_state == self.current_state):
		return;
		
	print_debug('Exiting State: ' + states.find_key(old_state))	
	current_state.exit();
	
	print_debug('Entering State: ' + new_state_name);
	new_state.enter();
